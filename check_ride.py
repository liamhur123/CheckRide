...
def shortfield_takeoff_dist(current_temp, pressure):
    global ground_data, obs_header, ground_header, obs_data
    ...
    for i in range(len(temps)):
        if temps[i] < current_temp < temps[i+1]:
            ...
            ground_header = ["{}°C".format(temps[i]),"{}°C".format(current_temp), "{}°C".format(temps[i+1])]
            ground_data = [[sea_lvl_grnd[i],(((sea_lvl_grnd[i+1] - sea_lvl_grnd[i]) * (current_temp % 10)/10) + sea_lvl_grnd[i]),sea_lvl_grnd[i+1]],
                    [low,middle,high],
            ...
            obs_header = ["{}°C".format(temps[i]),"{}°C".format(current_temp), "{}°C".format(temps[i+1])]
            obs_data = [[sea_lvl_obs[i],(((sea_lvl_grnd[i+1] - sea_lvl_obs[i]) * (current_temp % 10)/10) + sea_lvl_obs[i]),sea_lvl_obs[i+1]],
            ...
            data =[[sea_lvl_obs[i], (((sea_lvl_grnd[i + 1] - sea_lvl_obs[i]) * (current_temp % 10) / 10) + sea_lvl_obs[i]),sea_lvl_obs[i + 1]],[low_obs, middle_obs, high_obs],[one_thousand_ft_obs[i], (((one_thousand_ft_obs[i + 1] - one_thousand_ft_obs[i]) * (current_temp % 10) / 10) + one_thousand_ft_obs[i]), one_thousand_ft_obs[i + 1]]]
            print("OBS")
            #print(tabulate(data,headers=header,tablefmt="grid"))
            #print(data)
    return ground_data, obs_data, ground_header, obs_header
...
def main():
    ...
    if user == 1:
        ...
        return "Sorry, that was not an option!"
    print( "\nThanks hoe, bye.")
